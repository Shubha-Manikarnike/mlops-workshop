...
def go(args):
    ...
    for df, k in zip([trainval, test], ['trainval', 'test']):
        ...
        with open("tempfile","w") as fp:
            ...
            df.to_csv(fp.name+k+".csv", index=False)

            newArtifact = wandb.Artifact(name = f"{k}_data.csv", type = f"{k}_data" , description = f"{k}_data",metadata = {"key1":"value1"})
            newArtifact.add_file(fp.name+k+".csv")
            run.log_artifact(newArtifact)
            #log_artifact(
            #    f"{k}_data.csv",
            #    f"{k}_data",
